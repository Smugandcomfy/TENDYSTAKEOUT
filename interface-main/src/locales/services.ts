...
export const LaunchpadLocales: Locals = {
  has_withdrawn_already: t`You have withdrawn`,
  withdraw_to_investor_failed_owner_has_not_settled: t`The pool has not been settled`,
  withdraw_to_investor_failed_time_does_not_up: t`Haven't reach the withdraw date`,
  addParticipant_the_time_is_up: t`Deposit time is up`,
  You_are_not_in_whitelist: t`You are not in the whitelist`,
  insufficient_token_balance: t`Insufficient token balance`,
  The_time_is_out_of_bounds: t`Wrong time for settle`,
  The_available_time_is_out_of_range_when_add_investor: t`Out of deposit time`,
  The_available_time_is_out_of_range_when_append_wicp_from_investor: t`Out of deposit time`,
  Out_of_the_whitelist: t`You are not in the whitelist`,
  already_been_installed: t`Already been installed`,
  launchpad_manager_canister_has_been_installed: t`Launchpad manager has been installed`,
  The_launchpad_had_been_withdrawn: t`You have withdrawn`,
  The_launchpad_had_been_settled: t`The launchpad had not been settled`,
  The_time_of_launchpad_has_not_end_yet: t`The launchpad has not end yet`,
  Out_of_the_investor_whitelist: t`You are not in the whitelist`,
  duplicated_launchpad_name: t`The launchpad name must be unique. Please change the name and try again.`,
  "Denied!!": t`Denied!`,
};
...
export const getSwapMessage = (message: string | undefined): string | undefined => {
  return message ? SwapLocales[message] ?? message : undefined;
};

export const getLaunchpadMessage = (message: string | undefined): string | undefined => {
  return message ? LaunchpadLocales[message] ?? message : undefined;
};

export const getVoteMessage = (message: string | undefined): string | undefined => {
  return message ? VoteMessages[message] ?? message : undefined;
};

export const getXTCMessage = (message: string): string | undefined => {
  return message ? XTCMessages[message] ?? message : undefined;
};
...
export const getClaimMessage = (message: string): string | undefined => {
  return message ? ClaimMessages[message] ?? message : undefined;
};
