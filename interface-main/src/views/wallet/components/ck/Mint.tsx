...
import { useBTCDepositAddress, useUpdateBalanceCallback } from "hooks/useBTCCalls";
...
import { parseTokenAmount, ResultStatus } from "utils/sdk/index";
...
export default function MintCK_BTC({
  ...
  return (
    <>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            >
              ...
              <Button variant="outlined" size="small" disabled={balanceLoading} onClick={handelUpdateBalance}>
                {balanceLoading ? (
                  <CircularProgress size={18} sx={{ margin: "0 5px 0 0", color: "inherit" }}></CircularProgress>
                ) : null}
                <Trans>Update</Trans>
              </Button>
            </Box>
              ...
              <>
                <Box sx={{ background: theme.palette.background.level4, margin: "30px 0 0 0", padding: "10px" }}>
                  ...
                  <Box sx={{ margin: "15px 0 0 0", display: "flex", justifyContent: "center" }}>
                    <QRCode value={btc_address ?? ""}></QRCode>
                  </Box>
                </Box>
      ...
      <Box sx={{ margin: "20px 0 0 0" }}>
        <Transaction address={btc_address} block={block}></Transaction>
      </Box>
    </>
  );
}
