...
export class ICRC2Adapter extends BaseTokenAdapter<ICRC2Service> {
  ...
  public async transactions({ canisterId, params }: TransactionRequest) {
    ...
    if (_result) {
      ...
      const start = log_length - BigInt(params.offset!) - BigInt(1) - BigInt(params.limit!);
  ...
  public async approve({ canisterId, params, identity }: ApproveRequest) {
    return enumResultFormat<boolean>(
      await (
        ...
        fee: nullParamsFormat<bigint>(params.fee),
        created_at_time: [],
        amount: params.allowance * BigInt(10),
        memo: [],
        from_subaccount: nullParamsFormat<Array<number>>(params.subaccount ? params.subaccount : undefined),
      })
    );
  }
