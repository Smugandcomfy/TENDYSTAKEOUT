...
export function TokenItem({ canisterId }: { canisterId: string }) {
  ...
  const saveWalletCatchToken = useSaveCacheTokenCallback();

  const deleteWalletCatchToken = useDeleteCacheTokenCallback();

  const handleAddToken = (canisterId: string) => {
    saveWalletCatchToken([canisterId]);
  };

  const handleDeleteToken = (canisterId: string) => {
    deleteWalletCatchToken([canisterId]);
  };
  ...
  return (
    ...
    >
      <Grid container alignItems="center">
        <Avatar src={tokenInfo?.logo ?? ""}>&nbsp;</Avatar>
        <Grid ml={1} item xs>
