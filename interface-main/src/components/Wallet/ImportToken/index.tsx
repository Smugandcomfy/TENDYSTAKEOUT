...
export default function ImportTokenModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [metadata, setMetadata] = useState<null | undefined | TokenMetadata>(null);
  const [logo, setLogo] = useState<null | undefined | string>(null);
  ...
  const handleSubmit = async () => {
    ...
    const { valid, metadata, logo } = await standardCheck(values.id, values.standard as TOKEN_STANDARD);
    setLoading(false);
  ...
  const handleConfirm = () => {
    updateImportedToken(values.id, metadata!);
  ...
  const isExisted = (id: string | null | undefined) => {
    ...
    // if (id && importedTokens) {
    //   if (!!Object.keys(importedTokens ?? {}).find((importedId) => importedId === id)) return true;
    // }
  };

  let errorMessage = "";
  if (values.id && isValidPrincipal(values.id) && isExisted(values.id)) errorMessage = t`The token exists`;
  if (values.id && !isValidPrincipal(values.id)) errorMessage = t`Invalid canister id`;
  if (!values.id) errorMessage = t`Enter the canister id`;
  ...
  return (
    ...
    >
      ...
      {!checkFailed && metadata ? (
        <Box mt="40px">
          <Grid container>
            <Avatar sx={{ width: "48px", height: "48px", marginRight: "12px" }} src={logo ?? ""}>
              &nbsp;
            </Avatar>
            <Grid item xs>
      ...
      <Box mt="30px">
        <Button
          ...
          size="large"
          fullWidth
          disabled={!!errorMessage || loading || (!riskWarning && step === 1)}
          onClick={step === 1 ? handleConfirm : handleSubmit}
          startIcon={loading ? <CircularProgress size={24} color="inherit" /> : null}
        >
          {!!errorMessage ? errorMessage : step === 1 ? <Trans>Confirm</Trans> : <Trans>Import</Trans>}
        </Button>
      </Box>
    </Modal>
  );
